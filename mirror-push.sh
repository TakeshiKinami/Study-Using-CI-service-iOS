...
mirroring() {
    git clone --mirror git@github.com:TakeshiKinami/${SOURCE_REPOSITORY_NAME}.git
    cd ${SOURCE_REPOSITORY_NAME}.git
    git config user.name "TakeshiKinami"
    git config user.email "kinami.iphone@gmail.com"

    git remote set-branches origin 'heads/*'
    git remote set-url --push origin git@github.com:TakeshiKinami/${TARGET_REPOSITORY_NAME}.git
    git fetch -p origin
